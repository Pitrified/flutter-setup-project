...
void main() {
  ...
  group('StructuredOutputParser', () {
    test('parses pure JSON directly', () {
      ...
      expect(success.response.conversation.content, 'Muito bem!');
    });
    ...
    test('parses response with errors', () {
      ...
      expect(result, isA<ParseSuccess>());
      final success = result as ParseSuccess;
      expect(success.response.correction.errors, hasLength(1));
      expect(
        success.response.correction.errors.first.original,
        'Eu sou cansado',
      );
    });
  });
