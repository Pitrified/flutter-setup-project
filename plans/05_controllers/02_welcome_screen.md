...
App entry point screen. Shows app branding, current status, and a button to
start a new conversation. Redirects to model download if needed (handled by
router, not this screen).
...
- [ ] Shows start button when AppReady
- [ ] Shows error + retry when AppError
- [ ] Navigates to /conversation on button press
